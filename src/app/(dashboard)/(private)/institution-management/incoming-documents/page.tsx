...
import { incomingDocumentService, type IncomingDocument } from '@/services/incoming-document.service'
import { institutionService, type Institution } from '@/services/institution.service'
import { axiosClient } from '@/libs/axios'

const IncomingDocumentsPage = () => {
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true)
      setDocuments(await incomingDocumentService.getAll(selectedInstitution))
      setError(null)
    } catch (err: any) {
      setError(err.response?.data?.error?.message || 'Gelen evraklar yüklenirken bir hata oluştu')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteDocument = async () => {
    ...
    try {
      setDeleting(true)
      await axiosClient.put(`/api/incoming-documents/${selectedDocument.docId}`, { data: { document: null } })
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await incomingDocumentService.delete(id)
      await loadDocuments()
    } catch (err: any) {
  ...
  return (
    <>
      <Card>
        <CardHeader title="Gelen Evraklar" action={
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Geldiği Yer</TableCell>
                  <TableCell>Alınma Tarihi</TableCell>
              ...
              <TableBody>
                {documents.length === 0 ? (
                  <TableRow><TableCell colSpan={6} align="center">Henüz gelen evrak eklenmemiş</TableCell></TableRow>
                ) : (
                  documents.map((doc) => (
                    <TableRow key={doc.id}>
                      <TableCell>{doc.institution?.name || '-'}</TableCell>
                      <TableCell>{doc.title}</TableCell>
                      <TableCell>{doc.receivedFrom}</TableCell>
                      <TableCell>{doc.receivedDate ? new Date(doc.receivedDate).toLocaleDateString('tr-TR') : '-'}</TableCell>
                      <TableCell>
                        {doc.document ? (
      ...
      <Dialog open={dialogOpen} onClose={() => { setDialogOpen(false); setEditingId(null); }} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}><TextField fullWidth required label="Hangi Kurumdan Geldi" value={formData.receivedFrom} onChange={(e) => setFormData({ ...formData, receivedFrom: e.target.value })} /></Grid>
            <Grid item xs={12}><TextField fullWidth label="Alınma Tarihi" type="date" InputLabelProps={{ shrink: true }} value={formData.receivedDate} onChange={(e) => setFormData({ ...formData, receivedDate: e.target.value })} /></Grid>
